import {CONFIG,ENERGY_OFFERS,MAPS,NODE_BY_ID,ZONES,TOYS,SEQUENCE,TAIL,TASKS,STORY_REWARD,COLLECTION_REWARD,CATEGORY_REWARD,MANUAL_ITEMS,ITEM_BY_ID,MANUAL_REWARD} from './config.js';
...
function collect(s,id){if(!s.manual.includes(id)){s.manual.push(id);log(s,'manual_collect',{id});}}
...
export function advanceDialog(s,skip=false){
 ...
 if(d.type==='scene'){
  s.prepared[n.id]=true;for(const id of n.sceneItems||[])collect(s,id);
...
// A scene starts when the final prerequisite is committed. It never needs a submit/marker button.
export function resumeScene(s){
 ...
 if(s.prepared[n.id]){
  if(n.step<2){for(const id of n.sceneItems||[])collect(s,id);completeStory(s,n);}return;
 }
 const z=ZONES[n.map];
 const required=MANUAL_ITEMS.filter(i=>i.map===n.map&&i.step===n.step&&['pickup','material','ingredient'].includes(i.kind));
 if(required.some(i=>!ownsItem(s,i.id)))return;
...
function actCore(s,action,data={}){
 ...
 if(action==='node'){
  ...
  if(['material','ingredient','pickup'].includes(n.type)){
   ...
   return {ok:true,message:`${n.name}已放入任务手册`};
  }
  if(n.type==='supply'){s.completed[n.id]=true;reward(s,n.reward);return {ok:true,reward:n.reward};}
  if(n.type==='story'){
...
export function settle(s){if(s.ended)return {ok:true};if(!s.online)return fail('连接后才能确认活动结算');let coins=0,energy=s.energy;const mailed={};for(const m of MAPS)for(const n of m.nodes.filter(n=>n.type==='chest'))if(accessible(s,n)&&!done(s,n.id)){if(n.kind==='coins')coins+=n.amount;else energy+=n.amount;}
 for(const id of [...TASKS.map(t=>t.id),'manual-all','story']){const r=rewardFor(s,id);if(r.ready&&!s.claims.includes(id)){if(id==='manual-all'||id.startsWith('region-'))for(const [k,v]of Object.entries(r.base))s.legacyCredit[k]=Math.max(0,(s.legacyCredit[k]||0)-v);for(const [k,v]of Object.entries(r.reward)){if(k==='energy')energy+=v;else mailed[k]=(mailed[k]||0)+v;}}}
 const rate=s.review.energyConversion;const converted=typeof rate==='number'?Math.floor(energy*rate):0;coins+=converted;reward(s,{...mailed,coins:coins+(mailed.coins||0)});s.settlement={coins,mailed,pendingEnergy:rate===null?energy:0,converted,story:s.story.length,horses:s.horses.length,toys:s.toys.length};s.ended=true;s.energy=0;s.tickets=0;s.free=0;s.duplicate=0;s.manual=[];s.legacyCredit={};s.materials={};s.ingredients=false;s.horses=[];s.installed=[];s.toys=[];s.newToys=[];s.repairs=[];s.mementos=[];s.story=[];s.stages={};s.completed={};s.prepared={};s.sequence=0;s.dialog=null;s.pendingBatch=null;s.inside=false;log(s,'settlement');return {ok:true};}
